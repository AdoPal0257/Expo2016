      ...
      if($archivo['name'] != null)
        {
            $base64 = Validator::validateImage($archivo);
             $base6 = Validator::validateImage($archivo2);
            if($base64 != false)
            {
                $imagen = $base64;
                $wallpaper = $base6;
        ...
        if($nombre != "" && $apellidos != "")
        {
            if($id == null)
            {
                ...
                if($username != "" && $clave1 != "" && $clave2 != "")
                {
                    if($clave1 == $clave2)
                    {
                        ...
                        $sql = "INSERT INTO usuarios(Nombres, Apellidos, Email,Username, Clave, Fecha_nacimiento, Sexo, Tipo_Usuario, Foto_Usuario, Wallpaper) VALUES (?,?,?,?,?,?,?,?,?,?)";
                        $params = array($nombre, $apellidos, $correo, $username, $clave, $fecha_nacimiento, $sexo, $tipo, $imagen, $wallpaper);
                    }
                    else
                    {
            ...
            {
                $sql = "UPDATE usuarios SET Nombres= ?, Apellidos= ?, Email= ?, Fecha_nacimiento= ?, Sexo= ?, Tipo_Usuario= ?, Foto_Usuario= ?, Wallpaper= ? WHERE Id_Usuario= ?";
                $params = array($nombre, $apellidos, $correo, $fecha_nacimiento, $sexo, $tipo, $imagen, $wallpaper, $id);
            }
    ...
    <div class='input-field col s12 m6'>
      <div class="file-field input-field">
        <button class="btn red darken-2">
          ...
          <input type="file" name="wall" id="wall">
        </button>
        <div class="file-path-wrapper">
          <input class="file-path validate" name="wall" type="text" placeholder='1200x1200px máx., 2MB máx., PNG/JPG/GIF'>
        </div>  
      </div>
    </div>
  </div>
